                    ...
                    <div class="mt-6" x-data="{tab:1}">
                        ...
                        <div x-show="tab === 1"><b>People that follow you:</b>
                            <ul>
                                ...
                                <li>
                                    <a class="hover:text-stone-500" href="{{ route('posts.user', $follower->id) }}">{{ $follower->name }}</a>
                                </li>
                                @endforeach
                            </ul>
                        </div>
                        <div x-show="tab === 2"><b>People that you follow:</b>
                            <ul class="space-y-4">
                                ...
                                <li><a class="hover:text-stone-500" href="{{ route('posts.user', $following->id) }}">{{ $following->name }}</a><a href="{{ route('toggleFollow', $following) }}" class="ml-3 inline font-bold text-sm px-4 py-1 text-white rounded bg-blue-500 hover:bg-blue-600">
                                        Unfollow</a>
                                </li>
                                @endforeach
                            </ul>
                        ...
                        <div x-show="tab === 3"><b>Posts you liked:</b>
                            ...
                            @foreach (Auth::user()->likedPosts()->get() as $lpost)
                                <li>
                                    <a class="hover:text-stone-500" href="{{ route('posts.show', $lpost->id) }}">{{ $lpost->title }}</a>
                                    </li>
                            @endforeach
                            </ul>
                        </div>
