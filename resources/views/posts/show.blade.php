<x-microblog.layout>
    ...
    <div class="my-14 flex flex-col">
        <div class="text-center">
            ...
            <p class="italic text-sm">by {{ $post->user->name }} <img class="ml-2 object-scale-down h-14 w-14 rounded-full inline" src="{{ asset('storage/' . $post->user->image?->path) }}" alt="profile image"></p>
        ...
        <div class="flex mt-10">
            ...
            You follow:&nbsp;<a class="text-green-500 hover:text-green-700" href="{{ route('posts.user', $post->user->id) }}">
                {{ $post->user->name }}</a>
            @else
            <a href="{{ route('toggleFollow', $post->user) }}" class="ml-3 inline font-bold text-sm px-6 py-2 text-white rounded bg-blue-500 hover:bg-blue-600">
